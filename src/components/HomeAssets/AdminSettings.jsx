...
function AdminSettings({ isOpen, onClose }) {
  ...
  const handleLogin = (e) => {
    ...
    if (password === ADMIN_PASSWORD) {
      ...
      setError('Matrix Error: Unauthorized');
      setPassword('');
    }
  };
  ...
  const handleFileRead = (file, stateSetter) => {
    setUploading(true);
    const reader = new FileReader();
    reader.onloadend = () => { stateSetter(reader.result); setUploading(false); setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 3000);
    };
  ...
  const handleResumeChange = (e) => {
    const file = e.target.files[0]; if (file) handleFileRead(file, setResumePdf);
  };
  ...
  // --- ARRAY MANAGEMENT (Original Logic) ---
  const addEducation = () => {
  ...
  const updateArray = (arrName, idx, val, key) => {
    setProfileData(prev => ({ ...prev, [arrName]: prev[arrName].map((item, i) => i === idx ? (key ? { ...item, [key]: val } : val) : item) }));
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[999] flex items-center justify-center p-0 md:p-10 font-sans">
      <div className="relative w-full max-w-7xl h-full md:h-[90vh] flex flex-col md:flex-row bg-[#020617]/80 md:rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl animate-fade-in">
        ...
        {imageToCrop && (
          <div className="absolute inset-0 z-[1000] bg-black flex flex-col">
            <div className="p-8 flex justify-between items-center border-b border-white/10">
              <h3 className="text-white text-2xl font-black tracking-tighter uppercase italic">Avatar Sync</h3>
        ...
        {/* --- SIDEBAR --- */}
        <div className="w-full md:w-80 bg-black/40 border-r border-white/5 flex flex-col shrink-0 overflow-y-auto custom-scrollbar">
          <div className="p-10">
            <h2 className="text-2xl font-black text-white tracking-tighter uppercase italic">Console V2</h2>
            <div className="flex items-center gap-2 mt-2">
              <div className={`h-2 w-2 rounded-full ${isAuthenticated ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' : 'bg-red-500 shadow-[0_0_10px_#ef4444] animate-pulse'}`}></div>
              <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest">{isAuthenticated ? 'Authenticated' : 'Offline'}</span>
            </div>
          </div>
          ...
          {isAuthenticated && (
            <div className="flex md:flex-col px-4 gap-2 pb-10">
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-black transition-all duration-500 ${activeTab === tab.id ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 shadow-xl' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
          ...
          <div className="mt-auto p-8">
            <button onClick={saveAllData} disabled={!isAuthenticated} className="w-full py-5 bg-white text-black rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-cyan-400 transition-all shadow-2xl disabled:opacity-20">
        ...
        <div className="flex-1 flex flex-col bg-transparent relative overflow-hidden">
          <button onClick={onClose} className="absolute top-8 right-8 z-[50] p-4 bg-white/5 hover:bg-red-500/20 text-white hover:text-red-400 rounded-full transition-all border border-white/5">✕</button>

          <div className="flex-1 overflow-y-auto p-8 md:p-16 custom-scrollbar">
            {!isAuthenticated ? (
              <div className="h-full flex flex-col items-center justify-center animate-fade-in">
                <div className="w-32 h-32 bg-slate-900 rounded-[3rem] border border-white/5 flex items-center justify-center text-6xl shadow-inner mb-10">🔒</div>
                <form onSubmit={handleLogin} className="w-full max-w-sm space-y-6">
                  <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="ENTER MASTER KEY" className="w-full p-6 bg-white/5 border border-white/10 rounded-3xl text-white font-black tracking-[0.5em] text-center outline-none focus:border-cyan-500 transition-all placeholder:text-slate-800" autoFocus />
                  <button className="w-full py-6 bg-cyan-500 text-black font-black rounded-3xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all uppercase text-xs tracking-widest">Authorize Access</button>
                </form>
                {error && <p className="mt-6 text-red-500 font-black uppercase text-xs tracking-widest animate-bounce">{error}</p>}
              </div>
            ) : (
              <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-fade-in">
                {activeTab === 'personal' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <Input label="Current Semester" val={profileData.currentSemester} set={v => setProfileData({...profileData, currentSemester: v})} />
                    ...
                    <div className="md:col-span-2">
                       <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4 mb-2 block">Narrative Narrative</label>
                       <textarea className="w-full p-6 bg-white/5 border border-white/10 rounded-[2rem] text-white outline-none focus:border-cyan-500 transition-all min-h-[150px] resize-none leading-relaxed" value={profileData.description} onChange={e => setProfileData({...profileData, description: e.target.value})} />
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'journey' && (
                  <div className="space-y-6">
                    <button onClick={addEducation} className="w-full py-4 bg-white/5 border border-white/5 border-dashed rounded-2xl text-cyan-400 font-black uppercase text-xs tracking-widest hover:bg-white/10 transition-all">+ New Milestone</button>
                    {educationData.map((item, idx) => (
                      <div key={item.id} className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5 relative group hover:bg-white/[0.07] transition-all">
                        <div className="absolute top-8 right-8 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                           <button onClick={() => moveItem(idx, 'up')} className="p-2 bg-black/40 rounded-lg text-white">↑</button>
                           <button onClick={() => moveItem(idx, 'down')} className="p-2 bg-black/40 rounded-lg text-white">↓</button>
                           <button onClick={() => removeEducation(item.id)} className="p-2 bg-red-500/20 text-red-500 rounded-lg font-black">×</button>
                        </div>
                ...
                {activeTab === 'academic' && (
                   <div className="space-y-10">
                     <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5">
                        <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6">Annual CGPA Log</h4>
                        ...
                        <button onClick={() => addItem('yearData', { year: '', cgpa: '' })} className="text-cyan-400 font-bold text-xs">+ Append Year</button>
                     </div>
                   </div>
                )}
                ...
                {activeTab === 'skills' && (
                  <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5">
                    <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6">Technical Arsenal</h4>
                    <div className="flex flex-wrap gap-3">
                       ...
                       <button onClick={() => addItem('techStack', '')} className="p-2 bg-white/10 rounded-xl text-white font-black">+</button>
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'media' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="p-10 border-2 border-dashed border-white/10 rounded-[3rem] text-center space-y-6 hover:border-cyan-500 transition-all bg-white/[0.02]">
                      <div className="w-40 h-40 mx-auto rounded-full overflow-hidden border-[6px] border-white/5 shadow-2xl bg-slate-900">
                         {profilePic ? <img src={profilePic} className="w-full h-full object-cover" /> : <div className="h-full flex items-center justify-center text-5xl">👤</div>}
                      </div>
                      <input type="file" id="pic" className="hidden" onChange={handleProfilePicChange} accept="image/*" />
                      <label htmlFor="pic" className="inline-block px-8 py-3 bg-cyan-500 text-black rounded-2xl font-black text-[10px] cursor-pointer tracking-widest uppercase hover:bg-white transition-all">Update Avatar</label>
                    </div>
                    
                    <div className="p-10 border-2 border-dashed border-white/10 rounded-[3rem] text-center space-y-6 hover:border-blue-500 transition-all bg-white/[0.02]">
                       <div className="w-40 h-40 mx-auto rounded-[3rem] bg-slate-900 border border-white/5 flex items-center justify-center text-6xl shadow-2xl">📄</div>
                       <input type="file" id="resume" className="hidden" accept=".pdf" onChange={handleResumeChange} />
                       <label htmlFor="resume" className="inline-block px-8 py-3 bg-blue-600 text-white rounded-2xl font-black text-[10px] cursor-pointer tracking-widest uppercase hover:bg-white hover:text-black transition-all">Select PDF CV</label>
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* --- SYNC TOAST --- */}
        {showSuccess && (
          <div className="fixed bottom-12 right-12 bg-emerald-500 text-black px-10 py-5 rounded-[2rem] font-black tracking-widest uppercase text-xs shadow-2xl z-[9999] animate-bounce">
            ✨ Sync Successful
          </div>
        )}
      </div>
...
const Input = ({ label, val, set, type="text" }) => (
  <div className="group">
    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4 mb-2 block group-focus-within:text-cyan-400 transition-colors">{label}</label>
    <input type={type} value={val || ''} onChange={e => set(e.target.value)} className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-cyan-500 focus:bg-white/10 transition-all font-medium" />
  </div>
);
